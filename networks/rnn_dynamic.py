...
class RRNDynamicModel():
    ...
    def init_network(self):
        ...
        for i in range(1, len(dense_h_size)):
            self.W.append(tf.get_variable(self.namespace + "_W" + str(i), [dense_h_size[i - 1], dense_h_size[i]], dtype=tf.float32,
                                          trainable=self.is_training))
            self.b.append(tf.get_variable(self.namespace + "_b" + str(i), [1, dense_h_size[i]], dtype=tf.float32,
        ...
        if len(self.W) > 0:
            ...
            self.W_out = tf.get_variable(self.namespace + "_W_out", [self.config.hidden_size_rnn[-1], 1], dtype=self.data_type(),
                                         trainable=self.is_training)
            self.b_out = tf.get_variable(self.namespace + "_b_out", [1, 1], dtype=self.data_type(),
                                         trainable=self.is_training)
        ...
        while sample_count < batch_size:
            ...
            try:
                ...
                for i in range(point, point + trace_length):
                    tmp.append([self.buffer_x[type][bucket][episode][i], self.buffer_y[type][bucket][episode][i]])
            except IndexError:
