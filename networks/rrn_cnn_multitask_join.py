...
class GeneralRRNDiscreteModelMultitaskJointLoss():
    ...
    def translate_state(self, vx, vy, o_vx, o_vy, distance, dt):
        min_output = self.config.min_x
        max_output = self.config.max_x
        distance_translate = translate(distance, 0, self.config.max_sensor_distance, min_output, max_output)
        vx_t = translate(vx, -self.config.max_velocity, self.config.max_velocity, min_output, max_output)
        vy_t = translate(vy, -self.config.max_velocity, self.config.max_velocity, min_output, max_output)
        o_vx_t = translate(o_vx, -self.config.max_velocity, self.config.max_velocity, min_output, max_output)
        o_vy_t = translate(o_vy, -self.config.max_velocity, self.config.max_velocity, min_output, max_output)

        return [vx_t, vy_t, o_vx_t, o_vy_t, distance_translate, dt]
    ...
    def init_network(self):
        ...
        self.output_layer2 = tf.add(tf.matmul(rnn_output, self.W_out_2), self.b_out_2)
        ...
        with tf.name_scope("accuracy"):
            self.correct_pred1 = tf.less_equal(tf.abs(tf.argmax(self.Y1, 1) - self.predict1), 1)  # Count correct predictions
            self.correct_pred2 = tf.less_equal(tf.abs(tf.argmax(self.Y2, 1) - self.predict2), 1)  # Count correct predictions
        ...
        self.joint_loss = self.loss1 + self.loss2
        ...
        self.updateModel = self.trainer.minimize(self.joint_loss)
    ...
    def train_network(self, sess, learn_rate, log=False):
        ...
        state_train = sess.run([self.initial_state],
        ...
        if log:
            ...
            summary_str, _ = sess.run([self.sum_merge, self.updateModel], \
                                         feed_dict={self.observations: np.vstack(x),
                                                 ...
                                                 self.initial_state: state_train,
    ...
    def predict_angle_orientation(self, sess, s, old_rr_state):
        a, o, state1 = sess.run([self.predict1, self.predict2, self.rnn_state], \
    ...
    def validate(self, sess):
        ...
        # print "write stuff x: {0}".format(x)
        # print "write stuff y: {0}".format(y)
        accuracy1, accuracy2, summary_str, pred = sess.run([self.accuracy1, self.accuracy2, self.sum_validation, self.output_layer2], \
                                                     feed_dict={self.observations: np.vstack(x),
        ...
        #     print np.argmax(y2[i]), np.argmax(pred[i])
