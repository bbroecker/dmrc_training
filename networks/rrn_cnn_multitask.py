...
class GeneralRRNDiscreteModelMultitask():
    def __init__(self, num_drones, config, is_training, log_path, weight_path, namespace, variable_scope):
        ...
        with tf.name_scope(self.namespace + "_summaries"):
            summeries.append(tf.summary.histogram(namespace + "_weights_out_1", self.W_out_1))
            summeries.append(tf.summary.histogram(namespace + "_weights_out_2", self.W_out_2))
            summeries.append(tf.summary.histogram(namespace + "_biases_out_1", self.b_out_1))
            summeries.append(tf.summary.histogram(namespace + "_biases_out_2", self.b_out_2))
        if is_training:
            summeries.append(self.cost_sum1)
            summeries.append(self.cost_sum2)
            summeries.append(self.acc_sum1)
            summeries.append(self.acc_sum2)
    ...
    def init_network(self):
        ...
        self.W_out_1 = tf.get_variable(self.namespace + "_W_out_1",
                                       [self.config.hidden_size_rnn[0], self.config.output_size],
                                       dtype=self.data_type())
        self.W_out_2 = tf.get_variable(self.namespace + "_W_out_2",
                                       [self.config.hidden_size_rnn[0], self.config.output_size],
                                       dtype=self.data_type())
        self.b_out_1 = tf.get_variable(self.namespace + "_b_out_1", [1, self.config.output_size],
                                       dtype=self.data_type())
        self.b_out_2 = tf.get_variable(self.namespace + "_b_out_2", [1, self.config.output_size],
                                       dtype=self.data_type())
        ...
        self.observations_rrn = tf.reshape(self.observations,
                                           [self.tf_batch_size, self.trace_length, self.config.state_count])
        ...
        self.stacked_lstm1 = tf.contrib.rnn.MultiRNNCell([attn_cell() for _ in range(len(self.config.hidden_size_rnn))],
                                                         state_is_tuple=True)
        self.initial_state1 = self.stacked_lstm1.zero_state(self.tf_batch_size, self.data_type())
        rnn_output, self.rnn_state1 = tf.nn.dynamic_rnn( \
            inputs=self.observations_rrn, cell=self.stacked_lstm1, dtype=self.data_type(),
            initial_state=self.initial_state1,
            scope=self.namespace + '_rnn_1')
        self.stacked_lstm2 = tf.contrib.rnn.MultiRNNCell([attn_cell() for _ in range(len(self.config.hidden_size_rnn))],
                                                         state_is_tuple=True)

        self.initial_state2 = self.stacked_lstm2.zero_state(self.tf_batch_size, self.data_type())
        rnn_output2, self.rnn_state2 = tf.nn.dynamic_rnn( \
            inputs=self.observations_rrn, cell=self.stacked_lstm2, dtype=self.data_type(),
            initial_state=self.initial_state2,
            scope=self.namespace + '_rnn_2')

        rnn_output = tf.reshape(rnn_output, shape=[-1, self.config.hidden_size_rnn[0]])
        rnn_output2 = tf.reshape(rnn_output2, shape=[-1, self.config.hidden_size_rnn[0]])

        self.output_layer1 = tf.add(tf.matmul(rnn_output, self.W_out_1), self.b_out_1)
        self.output_layer2 = tf.add(tf.matmul(rnn_output2, self.W_out_2), self.b_out_2)

        self.predict1 = tf.argmax(self.output_layer1, 1)
        self.predict2 = tf.argmax(self.output_layer2, 1)

        self.Y1 = tf.placeholder(shape=[None, self.config.output_size], dtype=tf.float32)
        self.Y2 = tf.placeholder(shape=[None, self.config.output_size], dtype=tf.float32)
        ...
        with tf.name_scope("accuracy"):
            self.correct_pred1 = tf.equal(tf.argmax(self.Y1, 1), self.predict1)  # Count correct predictions
            self.correct_pred2 = tf.equal(tf.argmax(self.Y2, 1), self.predict2)  # Count correct predictions
            ...
            self.correct_pred1 = tf.boolean_mask(self.correct_pred1, tf.cast(mask_acc, tf.bool))
            self.correct_pred2 = tf.boolean_mask(self.correct_pred2, tf.cast(mask_acc, tf.bool))
            # self.shape_test = tf.shape(self.correct_pred1)
            self.accuracy1 = tf.reduce_mean(tf.cast(self.correct_pred1, "float"))  # Cast boolean to float to average
            self.accuracy2 = tf.reduce_mean(tf.cast(self.correct_pred2, "float"))  # Cast boolean to float to average
            # Add scalar summary for accuracy tensor
            self.acc_sum1 = tf.summary.scalar("accuracy1", self.accuracy1)
            self.acc_sum2 = tf.summary.scalar("accuracy2", self.accuracy2)
            validation1 = tf.summary.scalar("valid_accuracy1", self.accuracy1)
            validation2 = tf.summary.scalar("valid_accuracy2", self.accuracy2)
        ...
        self.error1 = tf.nn.softmax_cross_entropy_with_logits(logits=self.output_layer1, labels=self.Y1, name=None)
        self.error2 = tf.nn.softmax_cross_entropy_with_logits(logits=self.output_layer2, labels=self.Y2, name=None)
        ...
        self.error1 = tf.boolean_mask(self.error1, tf.cast(mask, tf.bool))
        self.error2 = tf.boolean_mask(self.error2, tf.cast(mask, tf.bool))

        self.loss1 = tf.reduce_mean(self.error1)
        self.loss2 = tf.reduce_mean(self.error2)
        self.cost_sum1 = tf.summary.scalar("angle_cost_function_1", self.loss1)
        self.cost_sum2 = tf.summary.scalar("angle_cost_function_2", self.loss2)
        self._lr = tf.Variable(0.0, trainable=False)

        self.trainer1 = self.gen_trainer(self.config.optimizer, self._lr)
        self.trainer2 = self.gen_trainer(self.config.optimizer, self._lr)
        self.updateModel1 = self.trainer1.minimize(self.loss1)
        self.updateModel2 = self.trainer2.minimize(self.loss2)
    ...
    def gen_init_state(self, sess, batch_size):
        init_state1, init_state2 = sess.run([self.initial_state1, self.initial_state2], feed_dict={self.tf_batch_size: batch_size})
        return init_state1, init_state2

    def train_network(self, sess, learn_rate, log=False):
        sess.run(self._lr_update, feed_dict={self._new_lr: learn_rate})
        state_train1, state_train2 = sess.run([self.initial_state1, self.initial_state2],
                                              feed_dict={self.tf_batch_size: self.config.batch_size})
        trainBatch = self.replay_buffer.sample(self.config.batch_size,
        ...
        if log:
            ...
            summary_str, _, _ = sess.run([self.sum_merge, self.updateModel1, self.updateModel2], \
                                      feed_dict={self.observations: np.vstack(x),
                                                 self.Y1: np.vstack(y1),\
                                                 self.Y2: np.vstack(y2),\
                                                 self.trace_length: self.config.trace_length,\
                                                 self.initial_state1: state_train1,
                                                 self.initial_state2: state_train2,
                                                 self.tf_batch_size: self.config.batch_size})
            self.summary_writer.add_summary(summary_str, self.log_count)
            self.log_count += 1
        else:
            _, _ = sess.run([self.updateModel1, self.updateModel2], \
                         feed_dict={self.observations: np.vstack(x),
                                    self.Y1: np.vstack(y1),\
                                    self.Y2: np.vstack(y2),\
                                    self.trace_length: self.config.trace_length,\
    ...
    def predict_angle_orientation(self, sess, s, old_rr_state1, old_rr_state2):
        a, o, state1, state2 = sess.run([self.predict1, self.predict2, self.rnn_state1, self.rnn_state2], \
                                        feed_dict={self.observations: [s], self.trace_length: 1,
                                                   self.initial_state1: old_rr_state1,
                                                   self.initial_state2: old_rr_state2, self.tf_batch_size: 1})
        return a, o, state1, state2

    def validate(self, sess):
        state_train1, state_train2 = sess.run([self.initial_state1, self.initial_state2],
                                              feed_dict={self.tf_batch_size: self.config.valid_batch_size})
        trainBatch = self.replay_buffer.sample(self.config.valid_batch_size,
                                               self.config.trace_length)  # Get a random batch of experiences.
        x = trainBatch[:, 0]
        y1 = trainBatch[:, 1]
        y2 = trainBatch[:, 2]
        ...
        # print "write stuff x: {0}".format(x)
        # print "write stuff y: {0}".format(y)
        accuracy1, accuracy2, summary_str = sess.run([self.accuracy1, self.accuracy2, self.sum_validation], \
                                                     feed_dict={self.observations: np.vstack(x),
                                                                self.Y1: np.vstack(y1), self.Y2: np.vstack(y2), \
                                                                self.trace_length: self.config.trace_length, \
                                                                self.initial_state1: state_train1,
                                                                self.initial_state2: state_train2,
                                                                self.tf_batch_size: self.config.valid_batch_size})
        self.summary_writer.add_summary(summary_str, self.valid_counter)
        self.valid_counter += 1
        return accuracy1, accuracy2

    def store_episode(self, state, y1, y2, bucket_id, type, episode):
        self.replay_buffer.add(state, y1, y2, bucket_id, type, episode)
        # self.replay_buffer.extend(
        #     np.reshape(np.array([state, angle]), [1, 2]))  # Save the experience to our episode buffer.

    def new_rrn_state(self, sess, state, state_rnn1, state_rnn2):
        return sess.run([self.rnn_state1, self.rnn_state2], \
                        feed_dict={self.observations: [state], self.trace_length: 1,
                                   self.initial_state1: state_rnn1,
                                   self.initial_state2: state_rnn2,
...
class ExperienceDualBuffer:
    ...
    def add(self, x, y1, y2, bucket_id, type, episode):
        ...
        self.buffer_y[type][bucket_id][-1].append([y1, y2])
    ...
    def sample(self, batch_size, trace_length):
        ...
        while sample_count < batch_size:
            ...
            try:
                ...
                for i in range(point, point + trace_length):
                    tmp.append([self.buffer_x[type][bucket][episode][i],
                                self.generate_y(self.buffer_y[type][bucket][episode][i][0]),
                                self.generate_y(self.buffer_y[type][bucket][episode][i][1])])
            except IndexError:
        ...
        result = np.reshape(sampledTraces, [batch_size * trace_length, 3])
        return result
