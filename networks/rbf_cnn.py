...
euclidean_dist_module = tf.load_op_library("/home/broecker/src/TensorFlowRBF/euclidean_dist.so")
...
class GeneralRBF_CNNModel():
    def __init__(self, config, is_training, namespace = ""):
        ...
        with tf.name_scope(self.namespace + "_activation_summaries"):
            summeries.append(tf.summary.histogram(namespace + "_activation_out", self.output_layer))
    ...
    def create_layer_rbf(self, prev_layer, W, b = None, threads = 1):
        if b is None:
            layer = RBFEuclidean(prev_layer, W, threads)
        else:
            layer = RBFEuclidean(prev_layer, W, threads) + b
    ...
    #     initial = tf.truncated_normal(shape, stddev=self.config.stddev)
    #     return tf.Variable(initial)
    #
    # def bias_variable(self, shape):
    #     initial = tf.constant(self.config.constant, shape=shape)
    #     return tf.Variable(initial)
    ...
    def init_network(self):
        ...
        self.observations = tf.placeholder(tf.float32, shape=[None, self.config.state_count * self.config.trace_length], name=self.namespace + "_observations")
        self.W_out = tf.get_variable(self.namespace + "_W_out", [h_size[-1], self.config.output_size], dtype=self.data_type())
        # self.W_out = self.weight_variable([h_size[-1], self.config.output_size])
        self.b_out = tf.get_variable(self.namespace + "_b_out", [1, self.config.output_size], dtype=self.data_type())
        # self.b_out = self.bias_variable([1, self.config.output_size])
        if self.is_training and self.config.keep_prob < 1:
            self.observations = tf.nn.dropout(self.observations, self.config.keep_prob)


        for i in range(1, len(h_size)):
            # self.W.append(self.weight_variable([h_size[i-1], h_size[i]]))
            # self.b.append(self.bias_variable([1, h_size[i]]))
        ...
        # self.layers = [self.create_layer_rbf(self.observations, self.W[0], None, 1)]
        # self.layers = [self.create_layer(self.observations, self.W[0])]
        self.layers = [self.create_layer(prev_layer=self.observations, W=self.W[0], b=self.b[0], use_tanh=self.config.use_tanh[0])]
        for i in range(1, len(self.W)):
            # with tf.name_scope(self.namespace + "_layer" + str(i)):
            self.layers.append(self.create_layer(self.layers[i-1], self.W[i], self.b[i], use_tanh=self.config.use_tanh[i]))
            # self.layers.append(self.create_layer_rbf(self.layers[i-1], self.W[i], self.b[i], 1))
            # self.layers.append(self.create_layer(self.layers[i-1], self.W[i]))
            # self.layers.append(self.create_layer(prev_layer=self.layers[i-1], W=self.W[i], b=None, use_tanh=self.config.use_tanh[i]))

        # with tf.name_scope("output_layer"):
        # self.output_layer = tf.add(tf.matmul(self.layers[-1], self.W_out), self.b_out)
        self.output_layer = RBFEuclidean(self.layers[-1], self.W_out, 1) + self.b_out
        ...
        self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=self.output_layer, labels=self.Y, name=None))
