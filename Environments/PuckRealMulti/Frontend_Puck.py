...
from tf_nn_sim.Environments.PuckRealMulti.Sprites.GeneralSprite import GeneralSprite
...
class PuckRealMultiFrontend:
    ...
    def draw(self, debug_data):
        ...
        for i in range(self.env.num_drones):
            x, y = self.meter_to_pixel(self.env.tx[i], self.env.ty[i])
            ...
            x, y = self.meter_to_pixel(self.env.ppx[i] - self.env.critical_radius,
                                       self.env.ppy[i] - self.env.critical_radius)
            ...
            x, y = self.meter_to_pixel(self.env.ppx[i], self.env.ppy[i])
            pygame.draw.circle(self.screen, color[i], [x, y], int(self.env.drone_radius * self.m_to_pixel_x))
            self.draw_line(BLACK, self.env.ppx[i], self.env.ppy[i],
                           self.wrap_angle(self.env.orientation[i]), self.env.drone_radius * 1.5)
            self.draw_line(BLACK, self.env.ppx[i], self.env.ppy[i],
                           self.wrap_angle(self.env.orientation[i] + np.pi), self.env.drone_radius)
            self.draw_line(BLACK, self.env.ppx[i], self.env.ppy[i],
                           self.wrap_angle(self.env.orientation[i] + self.env.velocity_angle[i]),
                           self.env.drone_radius * 2.0)
            self.draw_line(RED, self.env.ppx[i], self.env.ppy[i],
                           self.wrap_angle(self.env.orientation[i] + self.env.goal_angle[i]), self.env.goal_distance[i])
            if self.env.obstacle_angle[i] is not None:
                self.draw_line(BLUE, self.env.ppx[i], self.env.ppy[i],
                               self.wrap_angle(self.env.orientation[i] + self.env.obstacle_angle[i]),
                               self.env.obstacle_distance[i])
            # self.draw_line(BLUE, self.env.ppx[i], self.env.ppy[i],
            #                self.wrap_angle(self.env.orientation[i] + self.env.goal_angle[i]), self.env.goal_distance[i])
            ...
            if len(self.env.predict_obstacle_angle) > i:
                try:
                    self.draw_line(RED, self.env.ppx[i], self.env.ppy[i],
                                   self.wrap_angle(self.env.orientation[i] + self.env.predict_obstacle_angle[i]),
                                   self.env.drone_radius * 4.0)
                    self.draw_end_circle(RED, self.env.ppx[i], self.env.ppy[i],
                                         self.wrap_angle(self.env.orientation[i] + self.env.predict_obstacle_angle[i]),
            ...
            if len(self.env.predict_goal_angle) > i:
                try:
                    self.draw_line(BLUE, self.env.ppx[i], self.env.ppy[i],
                                   self.wrap_angle(self.env.orientation[i] + self.env.predict_goal_angle[i]),
                                   self.env.drone_radius * 4.0)
                    self.draw_end_circle(BLUE, self.env.ppx[i], self.env.ppy[i],
                                         self.wrap_angle(self.env.orientation[i] + self.env.predict_goal_angle[i]),
                                         self.env.drone_radius * 4.0, 0.025)
                except TypeError:
                    print "typeError :("
            ...
            if len(self.env.predict_goal_angle_noise) > i:
                try:
                    self.draw_line(GREEN, self.env.ppx[i], self.env.ppy[i],
                                   self.wrap_angle(self.env.orientation[i] + self.env.predict_goal_angle_noise[i]),
                                   self.env.drone_radius * 4.0)
                    self.draw_end_circle(GREEN, self.env.ppx[i], self.env.ppy[i],
                                         self.wrap_angle(self.env.orientation[i] + self.env.predict_goal_angle_noise[i]),
                                         self.env.drone_radius * 4.0, 0.025)
                except TypeError:
                    print "typeError :("
