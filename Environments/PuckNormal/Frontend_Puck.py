...
import tf_nn_sim.Environments.PuckNormal.Puckworld_Environment
...
class PuckNormalFrontend:
    # path update and path_lenght in seconds
    def __init__(self, env):
        assert isinstance(env, tf_nn_sim.Environments.PuckNormal.Puckworld_Environment.PuckNormalworld)
        self.env = env
    ...
    def draw(self):
        assert isinstance(self.env, tf_nn_sim.Environments.PuckNormal.Puckworld_Environment.PuckNormalworld)
        self.screen.fill(WHITE)

        x, y = self.pos_to_pixel(self.env.ppx, self.env.ppy)
        ...
        if self.env.angle_obstacle is not None:
            x_o = np.math.cos(self.env.angle_obstacle)
            y_o = np.math.sin(self.env.angle_obstacle)
        ...
        pygame.draw.circle(self.screen, color[1], [x, y], int(self.env.drone_radius * self.pos_to_pixel_x))

        x, y = self.pos_to_pixel(self.env.tx2 - self.env.bad_radius, self.env.ty2 - self.env.bad_radius)
        ...
        x, y = self.pos_to_pixel(self.env.tx2, self.env.ty2)
        pygame.draw.circle(self.screen, color[2], [x, y], int(self.env.drone_radius / 2 * self.pos_to_pixel_y))
