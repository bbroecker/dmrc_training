...
from main.logger.logger import DataLogger
from main.network_wrapper import AngleNetworkWrapper
from main.particle_filter.particle_filter_nn import ParticleFilter2_5D_Cfg
import collections
...
if __name__ == '__main__':
    ...
    movement_cfg = NNConfigMovement("../Config/NeuralNetwork/Movement/nn_config.yaml")
    ...
    for movement_cfg_training in [movement_cfg]:
        ...
        with tf.Graph().as_default():
            ...
            with tf.variable_scope("MovementModel", reuse=None, initializer=initializer_movement) as scope:
                training_network_movement = DQN(config=movement_cfg_training, namespace="DQN",
                                                is_training=True,
                                                log_path=movement_cfg_training.log_folder,
                                                weight_path=movement_cfg_training.weight_folder, variable_scope=scope,
                                                num_actions=env.num_actions,
                                                num_states=env.num_states)
            with tf.variable_scope("MovementModel", reuse=True, initializer=initializer_movement) as scope:
            ...
            with tf.Session() as sess:
                ...
                training_network_movement.init_logger(sess)
                ...
                n_d = 1
                max_num_drones = 2
                reward_avg = collections.deque(maxlen=3)
                while True:
                    n_d = 1 if n_d > max_num_drones else n_d
                    sim_cfg.num_drones = n_d
                    env = PuckRealMultiworldFinal(movement_cfg, sim_cfg, FRONTEND_ACTIVE, training_goal=False)
                    s = env.reset()
                    d = False
                    step_count = 0
                    ...
                    for step_count in range(int(env.max_epLength * env.update_rate)):
                        ...
                        epsi_per = (time.time() - start_time) / (movement_cfg_predict.epsilon_time * 60.0)
                        epsi_per = 1.0 if epsi_per > 1.0 else epsi_per
                        movement_cfg_training.epsilon = movement_cfg_training.epsilon_start - epsilon_range * epsi_per
                        ...
                        goal_collected += sum(goal_collect)
                        ...
                        if total_steps > 2:
                            for d_id in range(env.num_drones):
                                if total_steps % movement_cfg_training.store_every_ts == 0:
                                    training_network_movement.store_episode(s[d_id], actions[d_id], r[d_id], s1[d_id],
                                                                            crash[d_id] or goal_collect[d_id])
                        if episode_count > movement_cfg_training.pre_train_episodes and not training_worker.is_training:
                            print "start training!!"
                            training_worker.start_training(sess, env, training_network_movement, movement_cfg_training)
                        if training_worker.is_training:
                            training_worker.resume()

                        # logger.insert_data("reward", r)
                        rAll += sum(r) / env.num_drones
                        s = copy.copy(s1)
                        env.render(debug_d)
                        total_steps += 1
                    ...
                    if n_d == max_num_drones:
                        reward_avg.append(rAll / step_count)
                        if np.mean(reward_avg) > best_reward:
                            best_reward = np.mean(reward_avg)
                            print "save weights {}".format(best_reward)
                            training_network_movement.save_weights(sess, total_steps)
                    training_network_movement.store_rewards(sess, rewards, reward_log_count)
                    reward_log_count += 1
                    n_d += 1
                    episode_count += 1
                    ...
                    print "percent done: {0}%  epsilon {1} reward {2} update_rate {3}".format(
                        ...
                        np.mean(reward_avg), env.update_rate)
                    if percent >= 1.0:
                        break
                training_worker.stop_training()
